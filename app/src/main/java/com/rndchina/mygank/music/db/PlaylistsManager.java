package com.rndchina.mygank.music.db;
...
public class PlaylistsManager {
    ...
    public synchronized void insertMusic(Context context, long playlistid, MusicInfo info) {
        ...
        if (info.islocal) {
            if (albumart.equals(MusicUtils.getAlbumdata(App.context, info.songId))) {
    ...
    public synchronized void insertLists(Context context, long playlistid, ArrayList<MusicInfo> musicInfos) {
        ...
        for (int i = len - 1; i >= 0; i--) {
            ...
            if (info.islocal) {
                String art = MusicUtils.getAlbumdata(App.context, info.songId);
                if (art != null) {
                    break;
                } else {
                    albumart = null;
